...
const GoldTokenTopUp = ({ tokenToBuy, totalPrice, onDecreaseToken, onIncreaseToken }: TopUpTokenProps) => {
    const { t } = useTranslation();

    return (
        <View>
            <FastImage
                source={require('../../assets/images/Basic_Token_Illustration-big.png')}
                style={{ alignSelf: 'center', height: 160, width: 160 }}
                resizeMode={FastImage.resizeMode.contain}
            />
            <View className='my-4'>
                <Text className='self-center dark:text-white'>One token represents one job posting.</Text>
            </View>
